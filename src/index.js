...
async function main() {
  ...
  const panels = [health, feishuStatus, codex, ping, logs, syslog, deploy, frpc, applications];

  // collect all routes
  const routes = new Map();

  // dashboard routes
  routes.set('GET /', (req, res, url) => {
    const showAll = url.searchParams.get('show') === 'all';
    sendHtml(res, renderFull(config, panels, showAll));
  });
  routes.set('GET /api/html', (req, res, url) => {
    const showAll = url.searchParams.get('show') === 'all';
    sendHtml(res, renderInner(panels, showAll));
  });
  ...
  // start auth-gateway
  if (config.authGateway && config.authGateway !== false) {
    try {
